...
class Model_Vitrine extends \Orm\Model
{
	...
	protected static $_table_name = 'vitrine';

	protected static $_has_many = array(
	    'imagens' => array(
	   		'key_from' => 'id',
	        'model_to' => 'Model_Imagemvitrine',
	        'key_to' => 'id_vitrine'
	   	)
	);
	...
	public static function save_item($id = 0)
    {
		...
		try{
			...
			if($id){
				$item = Model_Vitrine::find($id);
				if(empty($item)){
					throw new Exception('nao-encontrado');
				}
			} else {
				$item = new Model_Vitrine();
			}
			...
			if(!empty($imagens) && is_array($imagens)){
				foreach($imagens as $keyI=>$itemDoc){
					...
					if(file_exists($pathImg) && !is_dir($pathImg)){
						$itemImg = new Model_Imagemvitrine();
						$itemImg->titulo = $itemDoc['titulo'];
						$itemImg->descricao = $itemDoc['descricao'];
						$itemImg->link = $itemDoc['link'];
						$itemImg->link_target = $itemDoc['link_target'];
						$itemImg->link_titulo = $itemDoc['link_titulo'];
    ...
    {	
    	if(!$status){
    		...
    		$item = Model_Vitrine::find('first', $cond);
    	}
    ...
    public static function get_total($ativo = false, $termo = null)
    {
    	...
    	$item = Model_Vitrine::find('all', $cond);
