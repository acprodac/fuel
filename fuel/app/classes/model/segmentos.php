...
class Model_Segmentos extends \Orm\Model
{
	...
	protected static $_table_name = 'segmentos';
	...
	public static function save_item($id = 0)
    {
		...
		try{
			...
			if($id){
				$item = Model_Segmentos::find($id);
				if(empty($item)){
					throw new Exception('nao-encontrado');
				}
			} else {
				$item = new Model_Segmentos();
				Model_Segmentos::increment_ordem();
				$item->ordem = 1;
			}
/*
			...
			foreach(self::$_properties as $key=>$elem){
				...
				} elseif($key == 'data_publicacao'){
					$dataPub = Input::post($key, null);
					list($dia, $mes, $ano) = explode('/', $dataPub);
					$dataPub = mktime(0, 0, 0, (int) $mes, (int) $dia, (int) $ano);
					$item->$key = $dataPub;
					$item->data_publicacao_dia = date('d', $dataPub);
					$item->data_publicacao_mes = date('m', $dataPub);
					$item->data_publicacao_ano = date('Y', $dataPub);
					continue;
				}
    ...
    public static function get_itens($limit = 0, $offset = 0, $ativo = false, array $order = array(), $termo = null, $home = null)
    {
    	$query = Model_Segmentos::query();
		
		if($home){
			$query->where('exibir_home', 1);
			$query->order_by('ordem', 'ASC');
			//$query->order_by('data_exibir_home', 'DESC');
		}
    ...
    {	
    	if(is_numeric($id)){
    		...
    		$item = Model_Segmentos::find('first', $cond);
    	}
    ...
    public static function get_total($ativo = false, $termo = null)
    {
    	...
    	$item = Model_Segmentos::find('all', $cond);
