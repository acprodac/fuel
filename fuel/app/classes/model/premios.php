...
class Model_Premios extends \Orm\Model
{
	...
	protected static $_table_name = 'premios';
	...
	public static function save_item($id = 0)
    {
		...
		try{
			...
			if($id){
				$item = Model_Premios::find($id);
				if(empty($item)){
					throw new Exception('nao-encontrado');
				}
			} else {
				$item = new Model_Premios();
				Model_Premios::increment_ordem();
				$item->ordem = 1;
			}
    ...
    public static function get_itens($limit = 0, $offset = 0, $ativo = false, array $order = array(), $termo = null, $home = false, $tipo = 'premios')
    {
    	$query = Model_Premios::query();
    ...
    {	
    	if(is_numeric($id)){
    		...
    		$item = Model_Premios::find('first', $cond);
    	}
    ...
    public static function get_total($ativo = false, $termo = null)
    {
    	...
    	$item = Model_Premios::find('all', $cond);
