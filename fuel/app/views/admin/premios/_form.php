<form id="formSave" class="well well-large form-page form-page-margin" action="<?php echo Uri::current(); ?>" method="POST" accept-charset="utf-8" data-redirect="<?php echo Uri::create('admin/premios'); ?>">
    <div class="formInit">
        ...
        <select id="baseArea" class="hide">
            <option rel="premios" value="premios" selected="selected">premios</option>
        </select>
        ...
        <div class="boxSlug boxSlugShow" rel="baseArea">
            <label for="itemSlug">url:</label>
            <div class="boxInfoSlug">
                <span class="infoSlug">Defina o título para gerar a URL.</span>
                <!-- <span class="urlForm"><?php echo $baseUrl; ?><span class="baseUrl"><?php echo empty($formCreate) ? 'premios/' : ''; ?></span></span> -->
                <span class="urlForm"><?php echo $baseUrl; ?><span class="baseUrl"><?php echo empty($formCreate) ? 'premios/' : ''; ?></span></span>
                ...
                <input type="text" name="slug" id="itemSlug" class="form required input-xlarge itemSlug itemSlugEdit" value="<?php echo empty($formCreate) ? $item->slug : ''; ?>" />
            </div>
            <span class="clear"></span>
        ...
        <label for="data_publicado">Data do prêmio:</label>
        ...
        <div class="boxTinyMce">
            <textarea name="texto" id="texto" class="form form-text input-xxlarge required tinyMce" data-height="300px"><?php echo !isset($formCreate) ? $item->texto : ''; ?></textarea>
        </div>
...
<!-- 
        <div class="box-item-galeria">
            ...
            <div class="box-cadastrar <?php echo isset($item->galeria->id) ? 'hide' : ''; ?>">
                <a data-fancybox-type="iframe" href="<?php echo Uri::create('admin/galeria/cadastrar?_c=frame'); ?>" class="btn btn-galeria-cad btn-info">Cadastrar galeria</a>
            </div>

            <div class="box-editar <?php echo !isset($item->galeria->id) ? 'hide' : ''; ?>">
                <a data-fancybox-type="iframe" href="<?php echo Uri::create('admin/galeria/editar/' . (isset($item->galeria->id) ? $item->galeria->id : '{ID}') . '?_c=frame'); ?>" class="btn btn-galeria-edit btn-info">Editar galeria <span ><?php echo isset($item->galeria->id) ? $item->slug : ''; ?></span></a>
