	<h3><a class="btnCadastrarNovo btn btn-success btnLink" href="<?php echo Uri::create('admin/vitrine/cadastrar'); ?>">Cadastrar nova vitrine</a>
    ...
    <div id="containner">
        ...
        <form id="formBuscaEmpresas" class="well well-large form-page<?php echo !empty($termoBusca) ? ' result' : ''; ?>" action="<?php echo Uri::current(); ?>" method="GET" accept-charset="utf-8">
            ...
            <label for="empresa">Procurar vitrine:</label>
            ...
            <?php if(!empty($termoBusca)) : ?>
                <a class="btn btn-link" href="<?php echo Uri::create('admin/vitrines'); ?>"><i class="icon-chevron-left"></i>Voltar</a>
            <?php endif; ?>
        ...
        <?php if(empty($itens)) : ?>
            ...
            <h4>Nenhuma vitrine <?php echo !empty($termoBusca) ? 'encontrada' : 'cadastrada'; ?>.</h4>
                ...
                <tbody>
                    <?php foreach($itens as $item) : ?>
                        <tr>
                            ...
                            <td class="center">
                                <a class="btn btn-<?php echo !empty($item->status) ? 'primary' : 'danger'; ?> btnStatus" href="<?php echo Uri::create('admin/vitrine/status/' . $item->id . '/' . (int) empty($item->status)); ?>" title="Clique para <?php echo !empty($item->status) ? 'desativar' : 'ativar'; ?>"><i class="glyphicon glyphicon-eye-<?php echo !empty($item->status) ? 'open' : 'close'; ?>"></i><?php echo !empty($item->status) ? 'ativo' : 'inativo'; ?></a>
                            <td class="center">
                                <div class="btn-group btnList">
                                    <a href="<?php echo Uri::create('admin/vitrine/editar/' . $item->id . '/'); ?>" class="btn btn-primary btnEditar btnLink"><i class="icon-edit icon-white"></i>Editar</a>

                                    <?php /* if($item->id > 21) : ?>
                                        <a href="<?php echo Uri::create('admin/vitrine/excluir/' . $item->id . '/'); ?>" class="btn btn-danger btnExcluir" rel="usuário"><i class="icon-remove icon-white"></i>Excluir</a>
                                    <?php endif;*/ ?>
                                </div>
