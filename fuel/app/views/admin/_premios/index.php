    <h3><a class="btnCadastrarNovo btn btn-success btnLink" href="<?php echo Uri::create('admin/premios/cadastrar'); ?>">Cadastrar nova prêmios</a>

        <?php if(!empty($itens) && count($itens) >= 2) : ?>
            <a href="<?php echo Uri::create('admin/premios/ordenar'); ?>" class="btn btn-primary btnOrdenar btnLink">Ordenar</a>
        <?php endif; ?>
    ...
    <div id="containner">
        ...
        <form id="formSave" class="well well-large form-page form-page-margin form-create" action="<?php echo Uri::current(); ?>" method="POST" accept-charset="utf-8">
                ...
                <input type="text" name="fields[destaque][0][titulo]" id="fields[destaque][0][titulo]" class="form input-xxlarge" value="<?php  echo $page->fields['destaque'][0]['titulo']; ?>" />

                <label for="fields[destaque][0][subtitulo]">Subtítulo:</label>
                <input type="text" name="fields[destaque][0][subtitulo]" id="fields[destaque][0][subtitulo]" class="form input-xxlarge" value="<?php  echo $page->fields['destaque'][0]['subtitulo']; ?>" />

                <label for="fields[destaque][0][texto]">Texto:</label>
                <div class="boxtinyMce tinyMce-min">
                    <textarea name="fields[destaque][0][texto]" id="fields[destaque][0][texto]" class="form form-text input-xxlarge required tinyMce tinyMce-min" data-height="150px"><?php  echo $page->fields['destaque']['0']['texto']; ?></textarea>
                </div>
        ...
        <form id="formBuscaEmpresas" class="well well-large form-page<?php echo !empty($termoBusca) ? ' result' : ''; ?>" action="<?php echo Uri::current(); ?>" method="GET" accept-charset="utf-8">
            ...
            <label for="empresa">Procurar premios:</label>
            ...
            <?php if(!empty($termoBusca)) : ?>
                <a class="btn btn-link" href="<?php echo Uri::create('admin/premios'); ?>"><i class="icon-chevron-left"></i>Voltar</a>
            <?php endif; ?>
        ...
        <?php if(empty($itens)) : ?>
            ...
            <h4>Nenhum premios <?php echo !empty($termoBusca) ? 'encontrado' : 'cadastrado'; ?>.</h4>
                ...
                <tbody>
                    <?php foreach($itens as $item) : ?>
                        <tr>
                            ...
                            <td class="center">
                                <a class="btn btn-<?php echo !empty($item->status) ? 'primary' : 'danger'; ?> btnStatus" href="<?php echo Uri::create('admin/premios/status/' . $item->id . '/' . (int) empty($item->status)); ?>" title="Clique para <?php echo !empty($item->status) ? 'desativar' : 'ativar'; ?>"><i class="glyphicon glyphicon-eye-<?php echo !empty($item->status) ? 'open' : 'close'; ?>"></i><?php echo !empty($item->status) ? 'ativo' : 'inativo'; ?></a>
                            <td class="center">
                                <div class="btn-group btnList">
                                    <a href="<?php echo Uri::create('admin/premios/editar/' . $item->id . '/'); ?>" class="btn btn-primary btnEditar btnLink"><i class="icon-edit icon-white"></i>Editar</a>

                                    
                                        <a href="<?php echo Uri::create('admin/premios/excluir/' . $item->id . '/'); ?>" class="btn btn-danger btnExcluir" rel="usuário"><i class="icon-remove icon-white"></i>Excluir</a>
